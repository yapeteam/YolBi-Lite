...
public class ImplScreen extends GuiScreen {
    ...
    public void initGui() {
        if (!init) {
            ...
            for (Module.category category : Module.category.values()) {
                int size = (int) YolBi.instance.getModuleManager().getModules().stream().filter(m -> m.moduleCategory() == category).count();
                if (size == 0) continue;
    ...
    public void update(int mouseX, int mouseY) {
        ...
        MainTheme[2] = ColorUtil.rainbow(10, 0, 1, 1, 1);//new Color(YolBi.instance.getModuleManager().getModule(ClientTheme.class).getColor(0));
    }
